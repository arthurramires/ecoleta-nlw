...
const Points = () => {
  ...
  useEffect(() => {
    async function loadPosition(){
      ...
      if(status !== 'granted'){
        Alert.alert('Ops!', 'Precisamos de sua permisssão para obter a localização');
        return;
      }
    ...
    <>
      ...
      <View style={styles.itemsContainer}>
        <ScrollView contentContainerStyle={{ paddingHorizontal: 20 }} horizontal showsHorizontalScrollIndicator={false}>
          {itens.map(item => (
            <TouchableOpacity  key={String(item.id)} style={[styles.item, selectedItens.includes(item.id) ? styles.selectedItem : {}]} onPress={() => handleSelectItem(item.id)}>
